...
func loadProductListFromEtcd(key string) error {
	...
	var secProductInfoList []*memory.SecProductInfoConf
...
func updatProducts(products []*memory.SecProductInfoConf) {

	m := make(map[string]*memory.SecProductInfoConf, len(products))

	for _, _p := range products {
		m[_p.ProductId] = _p
	}

	memory.Mem.Products.Lock.Lock()
	defer memory.Mem.Products.Lock.Unlock()

	memory.Mem.Products.ProductMap = m
